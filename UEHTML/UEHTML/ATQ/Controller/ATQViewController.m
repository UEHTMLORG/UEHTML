...
#import "ATQHaoyouViewController.h"
@interface ATQViewController ()<UIScrollViewDelegate>
...
@implementation ATQViewController
...
- (void)setupTitlesView
{
    ...
    //设置上面的按钮
    NSInteger width = (titlesView.width-60) / titleArr.count;
    NSInteger height = 40;
...
{
    [self setupOneChildViewController:ATQselectTCType];
    [self setupOneChildViewController:ATQselectHYType];
    [self setupOneChildViewController:ATQselectMEType];
    
}

- (void)setupOneChildViewController:(ATQselectType)type
{
    
    if (type == ATQselectTCType ) {
        ATQTongchengViewController *vc = [[ATQTongchengViewController alloc] init];
        
        [self addChildViewController:vc];
    }else if (type == ATQselectHYType){
        ATQHaoyouViewController *vc = [[ATQHaoyouViewController alloc] init];
        
        [self addChildViewController:vc];
    }else{
        ATQMyselfViewController *vc = [[ATQMyselfViewController alloc] init];
